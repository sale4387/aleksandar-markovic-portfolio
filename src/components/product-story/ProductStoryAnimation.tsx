...
export function ProductStoryAnimation({ story, title }: ProductStoryAnimationProps) {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setActiveIndex((current) => (current + 1) % steps.length), 1100);
      ...
      <MockAppFrame title={title} subtitle="Guided product walkthrough">
        <div className="flex h-full min-h-[15rem] flex-col justify-center">
          <div key={`${story.animationType}-${activeIndex}`} className="product-story-enter">
            ...
            {activeStep.loadingText && <LoadingPanel text={activeStep.loadingText} detail={activeStep.loadingDetail} />}
            {activeStep.output && <OutputPanel label={activeStep.label} value={activeStep.output} final={activeIndex === steps.length - 1} note={activeStep.finalNote} />}
          </div>
